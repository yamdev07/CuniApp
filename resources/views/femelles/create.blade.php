...
@section('title', 'Nouvelle Femelle - CuniApp Élevage')

@section('content')
<div class="page-header">
    <div>
        <h2 class="page-title">
            <i class="bi bi-plus-circle"></i> Nouvelle Femelle
        </h2>
...
<div class="cuni-card">
    <div class="card-header-custom">
        <h3 class="card-title">
            <i class="bi bi-info-circle"></i> Informations de la Femelle
        </h3>
    </div>
    <div class="card-body">
        <form action="{{ route('femelles.store') }}" method="POST" id="femelleForm">
            @csrf
            <div class="settings-grid">
                <div class="form-group">
                    ...
                    <div class="code-generator">
                        <input 
                            ...
                            data-check-url="{{ route('femelles.check-code') }}"
                        >
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nom" class="form-control @error('nom') is-invalid @enderror" 
                           value="{{ old('nom') }}" required placeholder="Ex: Lily">
                    @error('nom')
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="race" class="form-control @error('race') is-invalid @enderror" 
                           value="{{ old('race') }}" placeholder="Ex: Géant des Flandres, Rex">
                    @error('race')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="etat" class="form-select @error('etat') is-invalid @enderror" required>
                        ...
                        <option value="Gestante" {{ old('etat') == 'Gestante' ? 'selected' : '' }}>Gestante</option>
                        <option value="Allaitante" {{ old('etat') == 'Allaitante' ? 'selected' : '' }}>Allaitante</option>
                        <option value="Vide" {{ old('etat') == 'Vide' ? 'selected' : '' }}>Vide</option>
                    </select>
                    @error('etat')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('femelleForm').addEventListener('submit', function(e) {
        if (codeInput.classList.contains('is-invalid')) {
            e.preventDefault();
