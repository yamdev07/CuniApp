                ...
                <div class="settings-grid">
                            ...
                            <select name="femelle_id" class="form-select" required>
                                ...
                                @foreach ($femelles as $femelle)
                                    <option value="{{ $femelle->id }}"
                                        {{ old('femelle_id') == $femelle->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="saillie_id" class="form-select">
                                ...
                                @foreach ($saillies as $s)
                                    <option value="{{ $s->id }}"
                                        {{ old('saillie_id') == $s->id ? 'selected' : '' }}>
                                        {{ $s->femelle->nom ?? 'N/A' }} × {{ $s->male->nom ?? 'N/A' }}
                                        ({{ $s->date_saillie ? \Carbon\Carbon::parse($s->date_saillie)->format('d/m/Y') : 'N/A' }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="nb_vivant" class="form-control" value="{{ old('nb_vivant', 1) }}"
                                required min="0" max="20" id="nbVivant">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Nombre de mort-nés</label>
                            <input type="number" name="nb_mort_ne" class="form-control" value="{{ old('nb_mort_ne', 0) }}"
                                min="0" max="20" id="nbMortNe">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Total de la portée</label>
                            <input type="text" id="totalPortee" class="form-control" readonly
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="date_vaccination_prevue" class="form-control"
                                value="{{ old('date_vaccination_prevue') }}">
                        </div>
                        <div class="form-group">
...
// resources/views/naissances/create.blade.php - Add this script at the end
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const dateNaissance = document.querySelector('input[name="date_naissance"]');
            const dateSevrage = document.querySelector('input[name="date_sevrage_prevue"]');
            const dateVaccination = document.querySelector('input[name="date_vaccination_prevue"]');
            ...
            dateNaissance.addEventListener('change', function() {
                if (this.value) {
                    ...
                    // Sevrage: 6 weeks (42 days) after birth
                    const sevrageDate = new Date(birthDate);
                    sevrageDate.setDate(sevrageDate.getDate() + 42);
                    if (dateSevrage && !dateSevrage.value) {
                        dateSevrage.value = sevrageDate.toISOString().split('T')[0];
                    }

                    // Vaccination: 8 weeks (56 days) after birth
                    const vaccinationDate = new Date(birthDate);
                    vaccinationDate.setDate(vaccinationDate.getDate() + 56);
                    if (dateVaccination && !dateVaccination.value) {
                        dateVaccination.value = vaccinationDate.toISOString().split('T')[0];
                    }

                    // Set min dates for sevrage and vaccination
                    if (dateSevrage) {
                        dateSevrage.min = this.value;
                    }
                    if (dateVaccination) {
                        dateVaccination.min = this.value;
                    }
                }
            });
            ...
            document.querySelector('form').addEventListener('submit', function(e) {
                if (dateNaissance.value && dateSevrage.value) {
                    if (new Date(dateSevrage.value) < new Date(dateNaissance.value)) {
                        e.preventDefault();
                        alert('La date de sevrage doit être après la date de naissance');
                        dateSevrage.focus();
                        return false;
                    }
                }
                if (dateNaissance.value && dateVaccination.value) {
                    if (new Date(dateVaccination.value) < new Date(dateNaissance.value)) {
                        e.preventDefault();
                        alert('La date de vaccination doit être après la date de naissance');
                        dateVaccination.focus();
                        return false;
                    }
                }
            });
