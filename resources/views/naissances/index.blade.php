...
@section('content')
    ...
    {{-- ✅ STATS GRID --}}
    <div class="naissances-stats-grid"
                ...
                <div
                    ...
                    <div>
                        ...
                        <input type="text" name="search" value="{{ request()->get('search') }}" class="form-control"
                            placeholder="Nom ou code femelle..." style="border-radius: var(--radius-lg);">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="etat_sante" class="form-select" style="border-radius: var(--radius-lg);">
                            ...
                            <option value="Excellent" {{ request('etat_sante') === 'Excellent' ? 'selected' : '' }}>
                                Excellent</option>
                            <option value="Bon" {{ request('etat_sante') === 'Bon' ? 'selected' : '' }}>Bon</option>
                            <option value="Moyen" {{ request('etat_sante') === 'Moyen' ? 'selected' : '' }}>Moyen
                            </option>
                            <option value="Faible" {{ request('etat_sante') === 'Faible' ? 'selected' : '' }}>Faible
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="sex_verified" class="form-select" style="border-radius: var(--radius-lg);">
                            <option value="">Tous</option>
                            <option value="verified" {{ request('sex_verified') === 'verified' ? 'selected' : '' }}>
                                Vérifié</option>
                            <option value="pending" {{ request('sex_verified') === 'pending' ? 'selected' : '' }}>En
                                attente</option>
                        </select>
                    </div>
                    ...
                    <div style="display: flex; gap: 8px;">
                        ...
                        @if (request()->anyFilled(['search', 'date_from', 'date_to', 'etat_sante', 'sex_verified']))
                            <a href="{{ route('naissances.index') }}" class="btn-cuni secondary"
                                style="padding: 10px 16px;" title="Réinitialiser les filtres">
    ...
    <div class="cuni-card" style="width: 100%; max-width: 100%;">
        ...
        <div class="card-body" style="width: 100%; overflow-x: auto;">
                        ...
                        @forelse($naissances as $naissance)
                            <tr class="border-bottom border-light">
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <span class="badge"
                                        style="background: {{ $healthColors[$naissance->etat_sante] ?? $healthColors['Bon'] }}">
                                ...
                                <td>
                                    ...
                                    @elseif($naissance->reminder_count > 0)
                                        <span class="badge" style="background: rgba(239, 68, 68, 0.1); color: #EF4444;">
                                            <i class="bi bi-exclamation-triangle"></i> {{ $naissance->reminder_count }}
                                            rappel(s)
                                        </span>
                                    @else
                                ...
                                <td class="text-muted">
                                    ...
                                    @if (($naissance->jours_avant_sevrage ?? 0) > 0 && ($naissance->jours_avant_sevrage ?? 0) <= 7)
                                        <span class="badge"
                                            style="background: rgba(245, 158, 11, 0.1); color: #F59E0B; font-size: 10px;">
                                ...
                                <td class="pe-4">
                                    <div class="action-buttons">
                                        <a href="{{ route('naissances.show', $naissance) }}"
                                            class="btn-cuni sm secondary" title="Détails">
                                            <i class="bi bi-eye"></i>
                                        </a>
                            ...
                            <tr>
                                <td colspan="9">
                                    <div class="table-empty-state">
                                        ...
                                        <a href="{{ route('naissances.index') }}" class="btn-cuni secondary mt-2"
            ...
            @if ($naissances->hasPages())
                <div
                    style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--surface-border); flex-wrap: wrap; gap: 16px;">
