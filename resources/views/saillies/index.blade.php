                    ...
                    <tbody>
                        @forelse($saillies as $s)
                            <tr class="border-bottom border-light">
                                ...
                                <td class="text-muted">
                                    {{ $s->date_palpage ? date('d/m/Y', strtotime($s->date_palpage)) : '-' }}</td>
                                <td>
                                    @if ($s->palpation_resultat === '+')
                                        <span class="badge"
                                            style="background: rgba(16, 185, 129, 0.1); color: #10B981;">Positif</span>
                                    @elseif($s->palpation_resultat === '-')
                                        <span class="badge"
                                            style="background: rgba(239, 68, 68, 0.1); color: #EF4444;">Négatif</span>
                                    @else
                                        <span class="badge"
                                            style="background: rgba(107, 114, 128, 0.1); color: #6B7280;">En attente</span>
                                    @endif
                                </td>
                                <td class="pe-4">
            ...
            @if ($saillies->hasPages())
                <div
                    ...
                    <div class="text-muted" style="font-size: 13px;">
                        Affichage de <strong>{{ $saillies->firstItem() }}</strong> à
                        <strong>{{ $saillies->lastItem() }}</strong> sur <strong>{{ $saillies->total() }}</strong> saillies
                    </div>
                    @if ($saillies->hasPages())
