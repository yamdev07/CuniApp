...
@section('content')
    ...
    @else
            ...
            <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                @php
                    $stats = [
                        ...
                        [
                            'label' => 'Succès',
                            'count' => $notifications->where('type', 'success')->count(),
                            'color' => 'green',
                            'icon' => 'bi-check-circle',
                        ],
                        [
                            'label' => 'Alertes',
                            'count' => $notifications->where('type', 'warning')->count(),
                            'color' => 'amber',
                            'icon' => 'bi-exclamation-triangle',
                        ],
                        [
                            'label' => 'Infos',
                            'count' => $notifications->where('type', 'info')->count(),
                            'color' => 'blue',
                            'icon' => 'bi-info-circle',
                        ],
                    ];
        ...
        {{ $notif->is_read ? 'opacity-75' : 'notif-unread border-l-4 border-blue-500' }} 
        hover:bg-gray-50 dark:hover:bg-opacity-10 dark:hover:bg-gray-700">

                                <div class="flex items-start gap-3">
                                    <div class="mt-1 flex-shrink-0">
                                        <div class="w-9 h-9 rounded-full flex items-center justify-center icon-bg"
                                            data-type="{{ $notif->type }}"
                                            style="background: rgba({{ $notif->type === 'success'
                                                ? '16, 185, 129'
                                                : ($notif->type === 'warning'
                                                    ? '245, 158, 11'
                                                    : ($notif->type === 'error'
                                                        ...
                                                        : '59, 130, 246')) }}, 0.15)">
                                            <i
                                                class="bi {{ $notif->icon }} text-lg {{ $notif->type === 'success'
                                                    ? 'text-green-500'
                                                    : ($notif->type === 'warning'
                                                        ? 'text-amber-500'
                                                        : ($notif->type === 'error'
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex justify-between items-start">
                                            <h4 class="font-bold text-gray-800 dark:text-gray-100">{{ $notif->title }}</h4>
                                            <span class="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
                                                {{ $notif->created_at->diffForHumans() }}
                                            </span>
                                        </div>
                                        <p class="text-gray-600 dark:text-gray-300 mt-1 text-sm">{{ $notif->message }}</p>

                                        @if ($notif->action_url)
                                            <div class="mt-2 flex items-center gap-2">
                                                <a href="{{ $notif->action_url }}"
                                                    class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 group">
                                                    Voir les détails <i
                                    ...
                                    <div class="flex flex-col items-end space-y-1">
                                        @if (!$notif->is_read)
                                            <form action="{{ route('notifications.read', $notif->id) }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                                    class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 font-medium flex items-center gap-1">
                                                    <i class="bi bi-check-circle"></i> <span
                                                        class="hidden sm:inline">Marquer lu</span>
                                                </button>
                                            </form>
                                        @endif
