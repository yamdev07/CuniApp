...
<div class="cuni-card">
    ...
    <div class="card-body">
        <form action="{{ route('sales.update', $sale->id) }}" method="POST" id="saleForm">
            ...
            <div class="form-section" style="margin-top: 24px; border: 2px solid var(--primary-subtle); background: linear-gradient(135deg, var(--primary-subtle) 0%, var(--surface-alt) 100%);">
                <h4 class="section-subtitle">
                ...
                <div class="global-prices-grid" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 16px;">
                    {{-- Global Price: Mâles --}}
                    <div class="global-price-card" style="background: var(--surface); padding: 16px; border-radius: var(--radius-lg); border: 1px solid var(--surface-border);">
                        ...
                        <input type="number" id="globalPriceMales" class="form-control global-price-input" data-category="males" value="{{ old('global_price_males', \App\Models\Setting::get('default_price_male', 25000)) }}" min="0" step="100" placeholder="25000" style="font-weight: 600; font-size: 16px; border-color: #3B82F6;">
                        <small style="color: var(--text-tertiary); font-size: 11px; margin-top: 4px; display: block;">
                            Défaut: {{ number_format(\App\Models\Setting::get('default_price_male', 25000), 0, ',', ' ') }} FCFA
                        </small>
                    </div>
                    ...
                    <div class="global-price-card" style="background: var(--surface); padding: 16px; border-radius: var(--radius-lg); border: 1px solid var(--surface-border);">
                        ...
                        <input type="number" id="globalPriceFemales" class="form-control global-price-input" data-category="females" value="{{ old('global_price_females', \App\Models\Setting::get('default_price_female', 30000)) }}" min="0" step="100" placeholder="30000" style="font-weight: 600; font-size: 16px; border-color: #EC4899;">
                        <small style="color: var(--text-tertiary); font-size: 11px; margin-top: 4px; display: block;">
                            Défaut: {{ number_format(\App\Models\Setting::get('default_price_female', 30000), 0, ',', ' ') }} FCFA
                        </small>
                    </div>
                    ...
                    <div class="global-price-card" style="background: var(--surface); padding: 16px; border-radius: var(--radius-lg); border: 1px solid var(--surface-border);">
                        ...
                        <input type="number" id="globalPriceLapereaux" class="form-control global-price-input" data-category="lapereaux" value="{{ old('global_price_lapereaux', \App\Models\Setting::get('default_price_lapereau', 15000)) }}" min="0" step="100" placeholder="15000" style="font-weight: 600; font-size: 16px; border-color: #10B981;">
                        <small style="color: var(--text-tertiary); font-size: 11px; margin-top: 4px; display: block;">
                            Défaut: {{ number_format(\App\Models\Setting::get('default_price_lapereau', 15000), 0, ',', ' ') }} FCFA
                        </small>
                    </div>
                </div>
            ...
            <div class="form-section" style="margin-top: 24px; border: 2px solid var(--primary-subtle);">
                ...
                <div class="tab-content active" id="males-tab">
                    <div style="display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap;">
                        <input type="text" class="form-control" placeholder="Rechercher un mâle..." id="searchMales" style="flex: 1; min-width: 250px;" onkeyup="debouncedSearch('males', this.value)">
                    ...
                    <div class="rabbit-selection-grid" id="malesGrid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 12px;">
                        {{-- ✅ REPLACED FOREACH WITH PARTIAL INCLUDE --}}
                        @include('sales.partials.rabbit-grid', [
                            'rabbits' => $males,
                            'type' => 'males',
                            'soldIds' => $sale->rabbits()->where('rabbit_type', 'male')->pluck('rabbit_id')->toArray(),
                        ])
                    </div>
                    {{-- ✅ ADD PAGINATION INFO --}}
                    <div class="pagination-info" id="malesPaginationInfo" style="margin-top: 16px; text-align: center; color: var(--text-tertiary); font-size: 13px;">
                        Page {{ $males->currentPage() }} sur {{ $males->lastPage() }} ({{ $males->total() }} mâles au total)
                    </div>
                </div>
                ...
                <div class="tab-content" id="females-tab">
                    <div style="display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap;">
                        <input type="text" class="form-control" placeholder="Rechercher une femelle..." id="searchFemales" style="flex: 1; min-width: 250px;" onkeyup="debouncedSearch('females', this.value)">
                    ...
                    <div class="rabbit-selection-grid" id="femalesGrid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 12px;">
                        {{-- ✅ REPLACED FOREACH WITH PARTIAL INCLUDE --}}
                        @include('sales.partials.rabbit-grid', [
                            'rabbits' => $femelles,
                            'type' => 'females',
                            'soldIds' => $sale->rabbits()->where('rabbit_type', 'female')->pluck('rabbit_id')->toArray(),
                        ])
                    </div>
                    {{-- ✅ ADD PAGINATION INFO --}}
                    <div class="pagination-info" id="femalesPaginationInfo" style="margin-top: 16px; text-align: center; color: var(--text-tertiary); font-size: 13px;">
                        Page {{ $femelles->currentPage() }} sur {{ $femelles->lastPage() }} ({{ $femelles->total() }} femelles au total)
                    </div>
                </div>
                ...
                <div class="tab-content" id="lapereaux-tab">
                    <div style="display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap;">
                        <input type="text" class="form-control" placeholder="Rechercher un lapereau..." id="searchLapereaux" style="flex: 1; min-width: 250px;" onkeyup="debouncedSearch('lapereaux', this.value)">
                    ...
                    <div class="rabbit-selection-grid" id="lapereauxGrid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 12px;">
                        {{-- ✅ REPLACED FOREACH WITH PARTIAL INCLUDE --}}
                        @include('sales.partials.rabbit-grid', [
                            'rabbits' => $lapereaux,
                            'type' => 'lapereaux',
                            'soldIds' => $sale->rabbits()->where('rabbit_type', 'lapereau')->pluck('rabbit_id')->toArray(),
                        ])
                    </div>
                    {{-- ✅ ADD PAGINATION INFO --}}
                    <div class="pagination-info" id="lapereauxPaginationInfo" style="margin-top: 16px; text-align: center; color: var(--text-tertiary); font-size: 13px;">
                        Page {{ $lapereaux->currentPage() }} sur {{ $lapereaux->lastPage() }} ({{ $lapereaux->total() }} lapereaux au total)
                    </div>
                </div>
                ...
                <div style="margin-top: 24px; padding: 16px; background: var(--primary-subtle); border-radius: var(--radius-lg);">
                    ...
                    <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid var(--surface-border);">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 14px; font-weight: 600; color: var(--text-primary);"> 💰 Total de la vente: </span>
                            <span id="totalAmountDisplay" style="font-size: 20px; font-weight: 700; color: var(--primary);"> 0 FCFA </span>
                        </div>
                    </div>
                    <div id="quantityMismatchWarning" style="margin-top: 12px; padding: 12px; background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.3); border-radius: var(--radius); display: none;">
                        ...
                        <span style="color: var(--accent-orange); font-weight: 600;"> Veuillez entrer un prix pour chaque lapin sélectionné! </span>
                    </div>
                </div>
            </div>
...
(function() {
    ...
    function loadRabbits(type, page = 1, search = '') {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (paginationInfo) {
                    paginationInfo.innerHTML = `Page ${data.pagination.current_page} sur ${data.pagination.last_page} (${data.pagination.total} ${type} au total)`;
                }
    ...
    function loadMoreRabbits(button) {
        ...
        fetch('{{ route('sales.load-rabbits') }}', {
            ...
            body: JSON.stringify({
                type: type,
                page: page,
                search: document.getElementById('search' + type.charAt(0).toUpperCase() + type.slice(1)).value
            })
        })
        .then(response => response.json())
        .then(data => {
    ...
    function showToast(message, type = 'info') {
        ...
        const color = type === 'success' ? 'var(--accent-green)' : (type === 'error' ? 'var(--accent-red)' : 'var(--primary)');
        toast.innerHTML = `
