...
@forelse($rabbits as $rabbit)
    ...
    <label class="rabbit-card" style="display: flex; flex-direction: column; gap: 8px; padding: 12px; background: var(--surface-alt); border: 1px solid var(--surface-border); border-radius: var(--radius); cursor: pointer; transition: all 0.2s ease;" onclick="event.stopPropagation();">
        {{-- Checkbox Row --}}
        ...
        {{-- ✅ Price Input Container - Now visible when checked --}}
        <div class="price-input-container" id="price-{{ $category }}-{{ $rabbit->id }}" style="display: {{ $isSold ? 'block' : 'none' }}; margin-top: 8px;">
            ...
            {{-- ✅ Price input with ALL required data attributes --}}
            <input type="number" 
            ...
            <div class="price-indicator" id="price-indicator-{{ $category }}-{{ $rabbit->id }}" style="font-size: 10px; color: var(--accent-green); margin-top: 4px; display: {{ $isSold ? 'block' : 'none' }};">
...
@if($rabbits->hasMorePages())
    <div style="grid-column: 1 / -1; display: flex; justify-content: center; gap: 8px; margin-top: 16px;">
        <button type="button" class="btn-cuni secondary sm load-more-btn" data-type="{{ $type }}" data-page="{{ $rabbits->currentPage() + 1 }}" onclick="loadMoreRabbits(this)">
