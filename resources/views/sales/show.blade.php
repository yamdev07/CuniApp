...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="cuni-card" style="margin-top: 24px;">
                ...
                <div class="card-body">
                    <div class="grid grid-cols-3 gap-4">
                        <div
                            style="text-align: center; padding: 16px; background: rgba(59, 130, 246, 0.1); border-radius: var(--radius);">
                        ...
                        <div
                            style="text-align: center; padding: 16px; background: rgba(236, 72, 153, 0.1); border-radius: var(--radius);">
                        ...
                        <div
                            style="text-align: center; padding: 16px; background: rgba(16, 185, 129, 0.1); border-radius: var(--radius);">
            ...
            {{-- ✅ Lapins Associés avec Recherche --}}
            <div class="cuni-card" style="margin-top: 24px;">
                <div class="card-header-custom">
                    <h3 class="card-title">
                ...
                <div class="card-body">
                    ...
                    <div style="display: flex; gap: 12px; margin-bottom: 20px; flex-wrap: wrap;">
                        <input type="text" id="searchRabbits" class="form-control"
                            placeholder="Rechercher par nom ou code..." style="flex: 1; min-width: 300px;"
                            onkeyup="filterSaleRabbits()">
                        <select id="filterCategory" class="form-select" style="width: auto;" onchange="filterSaleRabbits()">
                            <option value="">Toutes les catégories</option>
                            <option value="male">Mâles</option>
                            <option value="female">Femelles</option>
                            <option value="lapereau">Lapereaux</option>
                        </select>
                    </div>
                    @if ($sale->rabbits->count() > 0)
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody id="saleRabbitsTable">
                                    @foreach ($sale->rabbits as $saleRabbit)
                                        <tr class="rabbit-row" data-category="{{ $saleRabbit->rabbit_type }}"
                                            data-code="{{ $saleRabbit->rabbit?->code ?? '' }}"
                                            data-name="{{ $saleRabbit->rabbit?->nom ?? '' }}">
                                            <td>
                                            ...
                                            <td>
                                                @if ($saleRabbit->rabbit_type === 'male')
                                                    <a href="{{ route('males.show', $saleRabbit->rabbit_id) }}"
                                                        class="btn-cuni sm secondary" title="Voir">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                @elseif($saleRabbit->rabbit_type === 'female')
                                                    <a href="{{ route('femelles.show', $saleRabbit->rabbit_id) }}"
                                                        class="btn-cuni sm secondary" title="Voir">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                @else
                                                    <a href="{{ route('lapins.edit', $saleRabbit->rabbit_id) }}"
                                                        class="btn-cuni sm secondary" title="Voir">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                @endif
                        ...
                        <div style="margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--surface-border);">
                            ...
                            <div style="margin-top: 12px;">
                                <form action="{{ route('sales.change-status', $sale) }}" method="POST">
                                    @csrf
                                    <select name="payment_status" class="form-select" style="margin-bottom: 8px;"
                                        onchange="this.form.submit()">
                                        <option value="pending"
                                            {{ $sale->payment_status === 'pending' ? 'selected' : '' }}>
                                            En attente
                                        </option>
                                        <option value="partial"
                                            {{ $sale->payment_status === 'partial' ? 'selected' : '' }}>
                                            Paiement partiel
                                        </option>
