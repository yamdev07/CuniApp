...
@section('content')
        ...
        <div class="card-body">
                            ...
                            <tr class="border-bottom border-light dark:border-gray-700">
                                ...
                                <td class="pe-4">
                                    <div class="action-buttons" style="display: flex; gap: 4px;">
                                        ...
                                        @if ($sale->payment_status !== 'paid')
                                            <form action="{{ route('sales.mark-paid', $sale) }}" 
                                                  method="POST"
                                                  id="mark-paid-form-{{ $sale->id }}" 
                                    ...
                                    @if ($canDelete)
                                        <form action="{{ route('sales.destroy', $sale) }}" 
                                            ...
                                            <button type="button"
                                                    ...
                                                    data-sale-info="{{ $sale->buyer_name }}"
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="table-empty-state">
                                        ...
                                        <p class="mt-3 text-gray-600 dark:text-gray-400">Aucune vente enregistrée pour le moment</p>
            ...
            @if ($sales->hasPages())
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--surface-border);">
                    ...
                    @if ($sales->hasPages())
                        <div class="cuni-card" style="margin-top: 0; border-top: none; border-radius: 0 0 var(--radius-lg) var(--radius-lg);">
    ...
    <div id="deleteConfirmModal" class="modal-overlay"
        ...
        <div class="modal-content"
            ...
            {{-- Header --}}
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px; padding-bottom: 16px; border-bottom: 1px solid var(--surface-border);">
                <div style="width: 40px; height: 40px; border-radius: 50%; background: rgba(239, 68, 68, 0.15); display: flex; align-items: center; justify-content: center;">
            ...
            <div style="margin-bottom: 24px;">
                ...
                <div style="background: rgba(239, 68, 68, 0.08); border-left: 3px solid var(--accent-red); padding: 12px 16px; border-radius: 0 var(--radius) var(--radius) 0; margin-bottom: 16px;">
                    <p style="margin: 0; font-size: 14px; color: var(--text-primary);">
                        ...
                        La vente <strong id="modalSaleInfo"></strong> et les <strong id="modalRabbitCount"></strong> lapin(s) associé(s) seront supprimés définitivement.
                    </p>
                </div>
            ...
            <div style="display: flex; gap: 12px; justify-content: flex-end;">
                <button type="button" class="btn-cuni secondary" onclick="hideDeleteModal()" style="padding: 10px 20px;">
    ...
    <style>
        ...
        .theme-dark .text-gray-800 { color: #F3F4F6 !important; }
        .theme-dark .text-gray-100 { color: #F9FAFB !important; }
        .theme-dark .text-amber-600 { color: #FCD34D !important; }
        .theme-dark .text-primary { color: #60A5FA !important; }
        .theme-dark .text-muted { color: #9CA3AF !important; }
        .theme-dark .fw-semibold { color: #E5E7EB !important; }
        .theme-dark .badge { opacity: 0.95; }
        .action-buttons { display: flex; gap: 4px; align-items: center; }
        .badge.bg-success { background: rgba(16, 185, 129, 0.15) !important; color: #10B981 !important; }
        .badge.bg-warning { background: rgba(245, 158, 11, 0.15) !important; color: #F59E0B !important; }
        .badge.bg-danger { background: rgba(239, 68, 68, 0.15) !important; color: #EF4444 !important; }
        .badge.bg-primary { background: rgba(59, 130, 246, 0.15) !important; color: #3B82F6 !important; }
        .badge.bg-secondary { background: rgba(107, 114, 128, 0.15) !important; color: #6B7280 !important; }
        ...
        .modal-overlay.closing .modal-content { animation: modalSlideOut 0.3s ease forwards; }
        .modal-content .btn-cuni.danger:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(239, 68, 68, 0.3); }
        @media (max-width: 480px) { .modal-content { padding: 20px 16px; margin: 16px; } .modal-content h3 { font-size: 16px; } }
    </style>
    @endpush
...
(function() {
    ...
    function showDeleteModal(form, saleInfo, rabbitCount) {
        console.log('🗑️ showDeleteModal called:', { form, saleInfo, rabbitCount });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function attachDeleteHandlers() {
            document.querySelectorAll('form[id^="delete-form-"] button[type="button"].danger').forEach(btn => {
                // Éviter les doublons
                ...
                if (modal.style.display !== 'flex') {
                    ...
                    buttons.forEach(btn => {
                        btn.onclick = function(e) {
                            e.preventDefault();
                            const form = this.closest('form');
                            if (form && confirm('⚠️ SUPPRESSION DÉFINITIVE\n\nÊtes-vous ABSOLUMENT sûr ?')) {
                                form.submit();
                            }
                        };
                    });
