...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            //  DEBUG : Voir la structure dans la console (à retirer en production)
            console.log('Événements:', events);
            console.log('naissances type:', typeof events.naissances, Array.isArray(events.naissances));
            ...
            //  FONCTION CORRIGÉE AVEC VÉRIFICATIONS
            function getEventsForDate(dateKey) {
                const result = [];

                // Saillies
                if (events.saillies && Array.isArray(events.saillies)) {
                    events.saillies.forEach(e => {
                        if (e.date === dateKey) {
                            result.push({
                                type: 'purple',
                ...
                // Naissances
                if (events.naissances && Array.isArray(events.naissances)) {
                    events.naissances.forEach(e => {
                        if (e.date === dateKey) {
                            result.push({
                                type: 'green',
                ...
                // Sexuations
                if (events.sexuations && Array.isArray(events.sexuations)) {
                    events.sexuations.forEach(e => {
                        if (e.date === dateKey) {
                            result.push({
                                type: 'blue',
