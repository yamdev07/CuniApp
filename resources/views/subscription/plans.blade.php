                ...
                ">
                    ...
                    ">
                                    <i class="bi bi-check-circle-fill"
                                        style="color: var(--accent-green); font-size: 14px;"></i>
                                    <span>{{ $feature }}</span>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <table class="table" style="width: 100%;">
                        <thead>
                            <tr style="border-bottom: 2px solid var(--surface-border);">
                                <th style="padding: 12px; text-align: left; font-size: 12px; color: var(--text-tertiary);">
                                    Date</th>
                                <th style="padding: 12px; text-align: left; font-size: 12px; color: var(--text-tertiary);">
                                    Montant</th>
                                <th style="padding: 12px; text-align: left; font-size: 12px; color: var(--text-tertiary);">
                        ...
                        <tbody>
                            @foreach ($paymentHistory as $payment)
                                <tr style="border-bottom: 1px solid var(--surface-border);">
                                    <td style="padding: 12px; font-size: 13px;">{{ $payment->created_at->format('d/m/Y') }}
                                    </td>
                            ...
                            ">{{ strtoupper($payment->payment_method) }}</span>
                                    ...
                                    <td style="padding: 12px; font-size: 13px;">
                                        @if ($payment->status === 'completed')
                                            <span style="color: var(--accent-green);"><i class="bi bi-check-circle"></i>
                                                Payé</span>
                                        @elseif($payment->status === 'pending')
                                            <span style="color: var(--accent-orange);"><i class="bi bi-clock"></i> En
                                                attente</span>
                                        @else
                                            <span style="color: var(--accent-red);"><i class="bi bi-x-circle"></i>
                                                Échoué</span>
                                        @endif
                                    </td>
                                </tr>
